...
- [SDK 전문가의 조언](#sdk-전문가의-조언)
   ...
   - [전문가의 조언3](#전문가의-조언3-합리적인-기본값을-사용해야-한다)
   - [전문가의 조언4](#전문가의-조언4-단순-래퍼wrapper-이상의-가치를-제공하라)
   - [전문가의 조언5](#전문가의-조언5-의미-있는-로그를-남기고-api의-복잡한-세부-사항은-숨겨야-한다)
...
#### 전문가의 조언3: 합리적인 기본값을 사용해야 한다.

SDK를 사용하는 대부분의 사용자는 설정파일이나 환경 구성을 건드리지 않고도 곧바로 실행되기를 기대한다.
이를 가능하게 한느 핵심 전략이 바로 합리적인 기본값(Sane-Default)이다. 합리적인 기본값이란 사용자가 별도의 매개 변수를 명시하지 않아도 SDK가 바로 동작하도록 해주는 설정을 말한다.

합리적인 기본값이란, 사용자가 별도의 매개 변수를 명시하지 않아도 SDK가 바로 동작하도록 해주는 설정을 말한다. 특히 중요한 기본값 중 하나는 API의 기본 URL이다. 
이 값은 SDK 내부에서 자동으로 설정되어 있어야 하며, 사용자가 문서를 읽지 않아도 바로 연결할 수 있어야 한다.

권장되는 기본값:
- 공개된 읽기 전용 API 라면, 운영 API 주소를 기본값으로 설정한다.
- 인증이 필요한 API 또는 읽기/쓰기 API라면, 실수나 데이터 손상을 방지하기 위해 샌드박스 환경 주소를 기본값으로 설정하는 것이 바람직하다.

이와 같은 기본값 구성은 전체 사용자 중 약 80%가 사용하는 표준적인 사용 흐름을 충실히 지원하며, 나머지 20%의 특수한 상황은 사용자가 직접 기본값을 덮어쓰는 방식으로 처리할 수 있도록 설계하면 된다. 이를 통해 SDK는 사용자 친화성과 유연성을 동시에 갖춘 도구로 거듭날 수 있다.

#### 전문가의 조언4: 단순 래퍼(Wrapper) 이상의 가치를 제공하라

좋은 SDK는 단순히 API를 호출하는 래퍼역할에 그쳐서는 안 된다. 최종 사용자, 즉 SDK를 사용하는 개발자의 생산성을 높여주는 도구로서의 기능을 제공해야 한다.

1. 강력한 데이터 유효성 검사
   사용자가 API 문서를 일일이 뒤져가며 데이터 타입을 맞추고 오류 검출 로직을 직접 짜는 것은 비효율적이다. API의 내부 구조를 가장 잘 아는 SDK 개발자가 이 검증 로직을 내장해 둔다면, 사용자는 잘못된 데이터를 보냈을 때 즉각적이고 명확한 피드백을 받을 수 있다.

2. 개발 도구(IDE)와의 완벽한 연동
   파이썬 SDK의 큰 강점은 서버 API 구현에 사용된 `Pydantic` 모델을 그대로 재활용할 수 있다는 점이다. 이를 통해 사용자는 IDE(VS Code, PyCharm 등)에서 자동 완성과 타입 힌트 기능을 100% 활용할 수 있다. 개발자는 `client.`을 입력하는 순간 사용 가능한 메서드와 필드를 추천받게 되며, 이는 오타를 줄이고 문서를 찾아보는 시간을 획기적으로 단축시킨다.

3. 복잡한 통신 로직의 추상화
   네트워크 통신 중 발생할 수 있는 일시적인 오류에 대한 재시도 로직이나, 인증 토큰 관리, 복잡한 에러 핸들링 등은 SDK 내부에서 처리되어야 한다. 사용자는 단지 `client.get_leagues()`라는 직관적인 함수 하나만 호출하면 되도록, 지루하고 반복적인 작업은 SDK가 대신 처리해주어야 한다.


#### 전문가의 조언5: 의미 있는 로그를 남기고, API의 복잡한 세부 사항은 숨겨야 한다.

SDK는 블랙박스처럼 동작해서는 안 되며, 사용자가 내부에서 어떤 일이 벌어지고 있는지를 파악할 수 있어야 한다. 
이를 위해 대표적인 방법은 두 가지다. 

첫째는 소스코드를 공개 저장소에 게시하는 것이고, 둘째는 의미 있는 로그 메시지를 통해 실행 흐름과 오류 발생 상황을 투명하게 보여주는 것이다.

로깅은 특히 사용자가 오류를 경험하거나 예상과 다른 결과를 받을 때 문제를 진단하는 데 매우 유용하다.

SDK의 사용자 대부분은 API 버전 관리, 인증, 속도 제한, 올바른 엔드포인트 탐색 같은 세부 사항에 신경 쓰고 싶어하지 않는다. 그들은 그저 어떤 함수를 호출해 자신들에게 의미 있는 작업을 하고 싶을 뿐이다. 이처럼 SDK는 내부 API의 복잡한 세부 구현을 사용자로부터 감추는 역할을 해야 한다.

예를 들어, `get_health_check()`와 `list_leagues()` 같은 메서드는 API 엔드포인트를 감싸는 래퍼 함수로, 복잡한 엔드포인트 이름이나 반환 타입 등을 신경 쓸 필요 없이 직관적인 함수 호출만으로 필요한 작업을 수행할 수 있도록 돕는다.
...
#### 2. 전체적인 흐름 설명
   ...
   0. 시작 전에 반드시 정하는 3가지 이름

      SDK 프로젝트는 이름이 3개 층위로 존재한다. 이걸 초기에 확정해야 이후가 깔끔하다.

      1. 레포/프로젝트 폴더명(로컬 디렉토리): 예) `acme-sdk/`
      2. 배포 이름(distribution name): `pip install ...`에 쓰는 이름
         - `pyproject.toml`의 `[project].name`
         - 예) `acme-sdk`
      3. import 이름(package name): 코드에서 `import ...`에 쓰는 이름
         - `src/<import_name>/` 폴더 이름
         - 예) `acme_sdk`

      관례:
         - 배포 이름: 소문자 + 하이픈(`acme-sdk`)
         - import 이름: 소문자 + 언더스코더(`acme_sdk`)

   
   1. 프로젝트 스캐폴딩(뼈대) 만들기

      1. 전형적인 디렉토리 구조(`src/` 레이아웃)

         실무에서 가장 흔한 클라이언트 중심 구조는 아래이다.
         ```
         acme-sdk/                          # [프로젝트 루트] Git 저장소의 최상위 폴더
         │
         ├── pyproject.toml                 # [설정 본부] 빌드 도구, 의존성 패키지, 배포 이름 정의 (가장 중요)
         ├── README.md                      # [대문] 패키지 소개, 설치법, 간단한 배지(Badge) 표시
         ├── LICENSE                        # [법적 고지] MIT, Apache 2.0 등 라이선스 명시
         ├── CHANGELOG.md                   # [역사 기록] 버전별 변경 사항(릴리스 노트) 관리
         │
         ├── src/                           # [Src 레이아웃] 소스 코드 격리 (테스트 코드와 혼입 방지)
         │   └── acme_sdk/                  # [임포트 패키지] 실제 코드 (import acme_sdk)
         │       ├── __init__.py            # [공개 API] 사용자가 쓸 Client, Error만 노출하고 내부는 숨김
         │       ├── client.py              # [핵심 로직] 사용자용 메인 클래스 (AcmeClient) 구현
         │       ├── errors.py              # [에러 정의] SDK 전용 예외 클래스 모음 (try-except용)
         │       ├── _http.py               # [내부 통신] HTTP 세션, 헤더, 재시도 로직 (사용자는 몰라도 됨)
         │       ├── _types.py              # [타입 정의] 내부에서 쓰는 데이터 구조, TypedDict, Enum 등
         │       └── py.typed               # [타입 마커] "이 패키지는 타입 힌트를 제공함"을 IDE에 알리는 빈 파일
         │
         ├── tests/                         # [테스트] 배포 패키지에는 포함되지 않는 테스트 코드
         │   └── test_smoke.py              # [기초 검증] 설치 및 임포트가 잘 되는지 확인하는 연기 테스트
         │
         └── examples/                      # [예제] 사용자가 바로 복사해 쓸 수 있는 스크립트
            └── quickstart.py              # [따라하기] 가장 기본적인 사용 시나리오 코드
         ```
         
   2. `pyproject.toml` 작성(배포 이름/빌드 백엔드/의존성의 중심)

      - `[build-system]`: 빌드 백엔드 지정(예: setuptools/hatchling)
      - `[project]`: 배포 이름/버전/의존성/파이썬 버전 등
   
      여기서 배포 이름은 `[project].name`이다.

      아래는 가장 보편적인 빌드 백엔드인 `setuptools` 기반 예시이다.
      ```
      [build-system]
      requires = ["setuptools>=68", "wheel"]
      build-backend = "setuptools.build_meta"
      ...
      핵심 포인트:
         - `src/` 레이아웃을 쓰므로 `package-dir`와 `find`로 패키지 탐색 위치를 `src`로 지정
         - `py.typed`를 포함시키기 위해 package-data 설정

   3. 개발 환경에서 설치(Editable install)로 개발 루프 만들기

      프로젝트 뼈대를 잡고 개발을 본격적으로 시작하기 전에 `pip install .`이나 `pip install -e .` 등을 하는 이유는 내 소스 코드를 현재 가상환경에 **설치된 패키지**로 등록해 실제 사용자와 가깝게 동일한 import·의존성 조건에서 실행·테스트할 수 있는 개발환경을 만들기 위해서이다. (실무에서는 대부분의 경우 `pip install -e .`을 사용해 개발 루프를 만든다.)
      > 단, 배포 직전에는 `python -m build`로 만든 wheel을 새 가상환경에 설치해(`pip install dist/*.whl`)해 배포물 기준으로 최종 검증한다.

      1. 왜 `pip install -e .`을 쓰나
         
         - `pip install .`: 복사 설치(현재 시점의 소스를 설치 위치로 복사/빌드)
         - `pip install -e .`: editable 설치(현재 작업 폴더를 참조하도록 연결) -> 소스 코드를 수정하면 재설치 없이 바로 반영

      2. `-e`를 적용하면 왜 바로바로 피드백이 되는가?

         이유: 코드가 복사본이 아니라 원본을 import하기 떄문이다.

         일반 설치(`pip install .`)는 보통 코드를 빌드하고 패키지를 wheel 형태로 만들어 나온 결과물을 `site-packages`로 복사/설치한다.
         
         그래서 설치 후에 소스를 수정해도 `site-packages`에 있는 설치본은 바뀌지 않으니 수정 사항을 반영하기 위해서는 다시 `pip install .`을 하여 재설치해야만 반영된다.

         반면, editable 설치(`-e` 기능 on)는 `site-package`에 코드를 복하지 않고 이 경로(프로젝트의 `src/`)를 import 경로로 추가하라는 식의 연결을 만들어 둔다. 

         그래서 파일을 수정하면, 파이썬이 다음 import/실행 시점에 바뀐 원본 파일을 읽게 되므로 재설치 없이 바로 반영된다.

         결론적으로 `pip install -e .`의 목적은 내 프로젝트를 실제 사용자처럼 `import`해서 쓸 수 있는 환경을 빠르게 구성하는 것이다. 즉, editable 설치는 테스트 가능한 설치 상태를 만들기 위한 개발 편의 장치이며, 빌드 백엔드는 그 설치 상태를 만들기 위해 간접적으로 관여할 수 있는 구성 요소이다.

   4. MVP 구현: 실사용 형태가 끝까지 연결되는 최소 단위

      개발 루프(`pip install -e .`)를 만들고 나면 이제 최소한의 SDK(MVP)를 실제로 테스트해봐야 한다.

      SDK의 MVP는 기능이 많을 필요가 없다. 딱 **하나의 요청이 성공하고, 실패 시 에러가 정상적으로 터지는지**만 확인하면 된다.

      즉, 이 단계의 목표는 기능을 많이 넣는 것이 아니라 사용자가 실제로 SDK를 쓰는 대표 흐름(인증 -> 요청 -> 응답 파싱 -> 오류 처리)이 끝까지 한 번이라도 제품 형태로 동작하도록 만드는 것이다.

      SDK에서 MVP는 다음을 만족해야 한다.
         - 사용자가 SDK를 설치한 것처럼 import 가능: `from my_sdk import Client`
         - Client 생성이 가능: 인증/기본 설정(베이스 URL, 타임아웃)
         - 최소 1개의 대표 엔드포인트 호출이 가능(보통 가장 많이 쓰이는 GET 1개가 정석)해야 한다.
         - 응답을 사용자가 쓰기 좋은 형태로 반환(dict 또는 모델)
         - 오류(4xx/5xx)를 일관된 예외 체계로 변환
         - 최소 스모크 테스트 1개 + 사용 예제 1개

      MVP 구현 직후의 다음 단계는 운영 전략에 따라 두 가지 버전(패턴)으로 나뉜다. 
      이때의 핵심 차이는 **MVP를 곧바로 공식 릴리스(`v0.1.0)로 배포하느냐** vs **조금 더 기능/품질을 쌓은 뒤 공식 릴리스를 하느냐**이다.
      > 속도(Velocity) vs 완성도(Completeness)
   
   5. 버전 A: MVP를 빠르게 배포하고(첫 릴리스) 그 다음부터 기능 확장 -> 재배포

      MVP가 끝까지 동작 하면 즉시 `v0.1.0`으로 배포하고, 사용자 피드백을 받아 `v0.2.0`, `v0.3.0`으로 반복 릴리스하는 방식이다.

      해당 패턴이 적합한 경우:
         - SDK를 쓸 실제 사용자(사내 팀, 고객, 다른 서비스)가 이미 있고, 빠른 피드백이 중요한 경우
         - API가 어느 정도 안정적이며, SDK의 기능을 따라가는 형태인 경우
         - 릴리스 프로세스(빌드/검증/배포)를 빨리 정착시키고 싶은 경우
         - 초기엔 기능이 적어도 괜찮고, DX/안정성 개선을 반복 릴리스로 쌓아갈 계획인 경우

      목적: MVP가 끝까지 동작하는 순간을 즉시 공식 릴리스(첫 배포)로 고정하여 설치/배포 파이프라인을 빨리 안정화하고 실제 사용자 환경(설치 후 import)에서 빠르게 피드백을 받아 이후 기능 확장을 안전하게 반복 릴리스로 진행

      0. `v0.1.0` Definition of Done (릴리스 합격 기준)

         버전 A에서는 빨리 배포가 핵심이지만, 이 기준은 반드시 충족해야 한다.

         1. 단위 테스트 최소 세트 통과
            - 성공 케이스 1개(대표 API)
            - 실패 케이스 2~3(예: 401/404/5xx)
         2. 패키징 검증 통과
            - `python -m build`로 wheel/sdist 생성
            - 새 가상환경에서 `pip install bist/*.whl` 후 `import my_sdk` 및 quickstart 실행 성공
         3. 문서/메타데이터 최소 기준
            - README에 설치/간단 사용법(Quickstart)
            - `pyproject.toml`에 `name/version/dependencies/requires-python` 명확

      1. 릴리스 준비 단계(코드 동결+메타데이터 정리)

         1. 버전 확정
            -`pyproject.toml`의 `[project].version`을 `0.1.0`으로 확정
            - 배포 이름과 import이름이 일관되는지 확인

         2. CHANGELOG 작성(강력 권장)
            
            `CHANGELOG.md` 예시(최소):
               - `0.1.0`에 무엇이 들어갔는지가 한 눈에 보이게
               예시:
                  - Added: Client, 인증 헤더, 대표 엔드포인트 1개
                  - Added: 기본 예외(ApiError/AuthError/NotFoundError 등)
                  - Added: Quickstart 예제

         3. README Quickstart 확정(배포 품질에 직결)

            README의 최소 구성:
               - 설치: `pip install <배포이름>`
               - 사용 예: `Client(...)`, `client.get_xxx(...)`
               - 에러 처리 예제 1개
      
      2. 품질 게이트 단계(테스트 중심)
         버전 A에서 가장 중요한 포인트는 배포가 빠르되, 최소 품질 기준은 자동으로 보장하는 것이다.

         1. 단위 테스트 확장(최소치)
            - 성공케이스 1개: 요청 경로/헤더/응답/파싱 검증
            - 오류 케이스 최소 3개:
               - 401/403 -> `AuthError`
               - 404 -> `NotFoundError`
               - 5xx -> `ServerError`(또는 `ApiError` 계열)
            - 가능하면 네트워크 오류(Timeout 등) 1개 추가 (선택)

            권장 방식: 실제 API 호출 대신 httpx MockTransport 등으로 결정론적 테스트 구성

         2. 로컬에서 테스트 실행
            대표적으로 `pytest` 가 있다.
            > 린트/포맷/타입체크는 버전 A에서도 도입하면 좋지만, 배포 속도가 중요하다면 최소테스트만으로도 출발 가능하다. 다만, 장기적으로는 빠르게 붙이는 것이 유리하다.

      3. 빌드 아티팩트 생성(배포물 만들기) - **핵심**

         1. 빌드 실행
            프로젝트 루트에서 `python -m build`를 실행한다.

            결과: `dist/` 폴더에 `*.whl`(wheel), `*.tar.gz`(sdist)가 생성된다.
         
         2. 해당 단계가 핵심인 이유
            `pip install -e .`는 개발 폴더를 직접 참조하기에 배포물에 포함되어야 할 파일이 누락돼도(예: 패키지 데이터, 설정 문제) 개발 중에는 티가 덜 날 수 있지만, 빌드(`python -m build`)는 설정된 파일만 엄격하게 골라내어 패키징하므로, 실제 배포 시 파일(데이터, 타입 힌트 등)이 누락되는 사고를 막기 위해 반드시 `python -m build`로 빌드 아티팩트를 만들어서 검증을 해야한다.

      4. 배포물(`wheel`) 설치 테스트 - 릴리스 전 필수 관문

         1. 새 가상환경에서 `wheel` 설치

            의도는 최종 사용자와 동일한 설치를 재현하는 것이다.

            절차:
               - 새 `venv`(가상환경) 생성
               - 그 `venv`에 `dist/*.whl` 설치 (`pip install dist/*.whl`)
               - `import` 및 `quickstart` 실행
            
            통과 기준:
               - `import my_sdk` 성공
               - README의 Quickstart 코드를 그대로 실행 가능
               - 대표 API 호출(또는 Mock 기반 예제)이 정상 동작

         2. 흔한 실패 유형(이 단계에서 자주 잡히는 것)
            - `src/` 레이아웃인데 빌드 설정이 잘못되어 패키지가 wheel에 안 들어감
            - `__init__.py/` 패키지 탐색 설정 문제로 import 실패
            - `py.typed`나 리소스 파일이 배포물에 누락
            - 의존성 누락(설치 후 런타임 에러)

      5. 배포(첫 릴리스 `v0.1.0`)

         여기서 **배포**는 보통 다음 중 하나이다.

         1. 내부 레지스트리 배포(사내 PyPI/아티팩트 저장소)
            - 가장 흔한 실무 형태
            - 팀/서비스들이 `pip install <배포이름>==0.1.0`으로 설치

         2. 공개(PyPI) 또는 테스트 레지스트리(TestPyPI)
            - 공개 배포는 첫인상이 중요하므로, 버전 A에서도 최소 문서/테스트는 갖추는 편이 좋다.
            - 절충안: `TestPyPI`로 먼저 검증 후 `PyPI` 공개

            배포 후 즉시 해야 하는 최소 검증: 
               - 완전히 새로운 `venv`에서 `pip install <배포이름>==0.1.0`
               - Quickstart 실행

      6. 배포 직후 단계(피드백 루프 시작)
         
         버전 A의 강점은 여기서 발생한다.

         1. 피드백 수집 포인트
            - 설치 과정에서 문제(의존성, 파이썬 버전)
            - 인증/타임아웃/프록시 환경 이슈
            - 예외 메시지/에러 처리 불편
            - 가장 필요로 하는 엔드포인트

         2. 핫픽스 기준(0.1.1 같은 PATCH)
            - 설치가 꺠짐
            - import가 깨짐
            - 인증/기본 호출이 깨짐
         
      7. 기능 확장 `v0.2.0` 재배포(반복 사이클)

         배포 후에는 동일한 릴리스 루프를 반복한다.

         1. 기능 확장의 범위(보통 MINOR)
            - 엔드포인트 추가
            - 페이지네이셔 도입
            - 재시도/백오프 옵션 추가
            - 문서/예제 확장(문서만이면 PATCH로도 가능)

         2. `v0.2.0` 릴리스 절차(핵심은 동일)
            1. 기능 구현 + 테스트 추가
            2. 버전 올림(`0.1.0` -> `0.2.0`)
            3. CHANGELOG 업데이트
            4. 빌드(`python -m build`)
            5. 새 `venv`에서 `pip install dist/*.whl` 설치 테스트
            6. 배포

            이후, `v0.3.0`, `v0.4.0`... 같은 방식으로 반복한다.
         
      
      > 버전 A를 쓰는 현실적인 이유: SDK는 설치해서 쓰는 제품이므로 최대한 빨리 실제 설치 환경에서의 문제와 요구를 드러내고, 이를 버전 단위로 반복 개선하는 것이 가장 비용 효율적이기 떄문이다.

   6. 버전 B: MVP는 내뷍서 계속 확장/정비하고, 일정 수준 후 첫 공식 배포

      MVP 이후에도 당장 공식 배포는 하지 않고, 핵심 기능/품질을 더 채운 뒤 첫 공식 릴리스(`v0.1.0`)를 비교적 완성도 있게 내는 방식이다.

      해당 패턴이 적합한 경우:
         - 외부 공개(PyPI)처럼 첫 인상이 중요한 경우
         - API가 아직 변동이 큰 상태라, MVP를 배포해도 곧 깨질 가능성이 높은 경우
         - 조직 정책상 릴리스 최소 요건이 높아 MVP만으로는 배포가 허용되기 어려운 경우
         - 내부 개발자만 쓰며, 당장은 설치 배포보다 코드 기반 공유(레포 참조)가 가능한 경우

      목적: MVP가 동작하는 것만으로는 부족하여(외부 공개, 첫인상 중요, 조직 정책 등) 핵심 기능/품질/문서/테스트를 일정 수준까지 갖춘 뒤 첫 공식 배포를 완성도 있게 내는 전략

      0. 버전 B의 핵심: `v0.1.0` 릴리스 게이트를 먼저 문서로 고정

         버전 B에서 가장 중요한 실무 원칙은 그럴싸해질 떄까지가 아니라 특정 기준을 통과하면 `v0.1.0`으로 낸다를 먼저 정해둔다는 것이다.

         아래는 SDK에서 현실적으로 가장 많이 쓰는 `v0.1.0` 최소 기준이다.

         1. 기능 스코프(제품 관점)
            - 핵심 리소스 1개 이상(예: users, projects 등) 기준으로 최소 `GET` 1개 + `LIST` 1개 + (가능하면) `CREATE` 1개 이상씩 존재
            > , 사용자가 써볼 가치가 있는 사용 흐름이 2~3개는 있어야 한다.      

            - 인증/기본 설정(예: `base_url`, `api_key`, `timeout`)이 안정적
         ...
         2. 오류/예외 체계(운영 관점)
            ...
            - 예외 객체에 최소 포함: `status_code`, `message`, `request_id`

         3. 신뢰성(네트워크 관점)
            - timeout 옵션 제공(기본값 포함)
            - 429/503 같은 일시 오류에만 `idempotest` 요청(`GET`) 중심
            - 기본값은 보수적으로(또는 기본 off) 설계

         4. 테스트(배포 품질 포함)
            - 단위 테스트로 성공 1개 + 실패(401/404/5xx) 최소 3개는 확보
            - 배포물 기준 검증(필수)
               - `python -m build` 성공
               - 새 `venv`에서 `pip install dist/*.whl` 성공
               - README quickstart를 그대로 실행해서 성공
         ...
         대부분의 경우 이걸 통과하면 `v0.1.0`을 낼 수준이라고 말할 수 있다.

         릴리스 게이트 문서를 정의하였다면, `v0.1.0` 스코프를 확정한다.(기능 범위 고정)
         - `v0.1.0`에 넣을 것/넣지 않을 것을 확정한다.


      1. 내부 반복 개발 루프를 릴리스 후보 방식으로 운영

         버전 B의 핵심은 공식 배포는 안 하지만, 내부적으로는 리리스와 동일한 규율로 반복하는 것이다.

         - 작업 단위는 **기능 + 테스트 + 문서**를 한 묶음으로 각 기능(엔드포인트 1개 추가 등)을 다음 세트로 처리한다.

            1. 기능 구현
            2. 단위 테스트 추가(성공/실패 케이스 최소 포함)
            3. 문서/예제 업데이트(가능한 범위)
            4. CI 통과 확ㅇ니

         통과 기준(권장):
            - 기능 PR(또는 커밋)이 합쳐지기 전에 테스트가 반드시 추가됨
            - 동작만 하는 코드가 아니라 검증 가능한 코드로 누적

         이유: 버전 B는 공식 배포가 늦기 떄문에, 품질 부채가 쌓이면 막판 폭발한다.
      
      2. 버전 표기와 브랜치 전략을 먼저 잡아 릴리스 준비 상태를 관리

         버전 B는 아직 공식 배포 전 상태를 명확히 표현해야 한다.

         1. 프리릴리스/개발 버전 사용(PEP 440)
            
            다음 중 하나를 운영한다.
            - 개발 스냅샷: `0.1.0.dev1`, `0.1.0.dev2`, ...
            - 알파/베타: `0.1.0a1`, `0.1.0b1`, ...
            - 릴리스 후보: `0.1.0rc1`

            권장 운영 흐름
            - 내부 개발 기간: `0.1.0.devN`
            - 기능 스코프 거의 완료: `0.1.0b1`
            - 배포 직전 안정화: `0.1.0rc1`
            
            이유: 정식과 준비 중을 기술적으로 구분할 수 있어 설치/업그레이드 정책이 명확해진다.

         2. 브랜치 운영(실무형, 선택)

            - `main`: 안정(릴리스 가능한 상태)
            - `develop`: 개발 통합(선택)
            - `release/0.1.0`: `v0.1.0` 안정화 적용 브랜치(권장)
            - `hotfix/*`: 배포 후 긴급 수정
            
            버전 B에서는 특히 `release/0.1.0` 브랜치를 만들어 스코프 완료 이후는 버그 수정만이라는 규율을 만들면 좋다.

      3. 배포물 기준 검증을 공식 배포 전에도 정기적으로 수행

         이게 버전 B에서 가장 중요한 실행 단계이다. (**많이 터지는 이슈를 막아준다**)

         1. 정기 빌드(아티팩트 생성)
            - `python -m build`로 `dist/*.whl`, `dist/*.tar.gz` 생성

         2. 새 가상환경에서 wheel 설치 테스트 (필수)
            - 새 가상환경 `venv` 생성
            - `pip install dist/*.whl` 실행
            - README quickstart 그대로 실행

            통과 기준:
               - `import my_sdk` 성공
               - quickstart 예제 실행 성공
               - 의존성 누락/패키지 누락/리소스 누락 없음

            이유: `pip install -e .`는 작업 트리를 직접 참조하기에 배포물 누락을 잡아낼 수 없다. 버전 B의 경우 작업 시간이 길기에 배포물 누락이 빈번히 발생할 수 있다.

         3. 빈번한 실패 유형 (버전 B에서 자주 발생하는 케이스들)
            - `src/` 패키지가 wheel에 안 들어감(패키지 탐색 설정 문제)
            - `py.typed/` 리소스 파일이 배포물에 누락됨
            - README/메타데이터 렌더링 문제(특히 공개 배포 시)
            - 의존성 누락(개발환경에는 있었는데 배포 메타데이터에 없어서 설치 후 실패)

      4. 공식 배포 전 설치 경험을 확보하는 내부 배포/테스트 채널 운영

         공식 배포를 늦추면 현실 피드백이 늦어진다. 이 단계를 이를 보완하는 것이다.

         1. 내부 설치 채널 3가지 중 하나를 선택
            - VCS 설치(가장 간단): `pip install git+...@tag/commit`
            - 사내 테스트 레지스트리: `0.1.0.devN` 또는 `0.1.0rc1` 업로드
            - TestPyPI: 공개 PyPI 전 설치·메타데이터·의존성 검증

         통과 기준(권장):
            - 내 PC가 아닌 다른 환경(동료, CI runner, 다른 OS)에서 설치 성공
            - 실제 사용자의 불편(인증 설정, 프록시, 타임아웃, 에러 메시지)을 최소 1회 이상 수집

         
      5. 스코프 완료 후: 릴리스 후보(RC) 단계로 전환

         기능이 어느 정도 다 채워졌다면, 여기서부터는 개발의 성격이 바뀐다.

         1. 코드 프리즈(Code Freeze)
             - `rc1` 이후에는 기능 추가 금지
             - 허용: 버그 수정, 문서 보완, 테스트 보강, 패키지 수정

            이유: 기능을 계속 넣으면 안정화가 영원히 끝나지 않는다.

         2. 회귀 테스트 강화(변경 영향을 잡기)
            - 핵심 시나리오(quickstart)를 릴리스 전 필수 테스트로 고정
            - 에러 매핑/인증/페이지네이션 같은 공통 기능은 회귀 테스트를 반드시 둠

            통과 기준: RC에서 새 이슈가 나오면 `rc2`, `rc3`로 반복하되 범위는 버그 수정에만 제한

      6. `v0.1.0` 공식 배포 실행(버전 B의 결승선)

         RC가 안정화되면 이제 정식 릴리스로 전환한다.

         1. `v0.1.0` 버전 확정 및 산출물 고정

            - 버전을 `0.1.0`으로 확정
            - CHANGELOG/README 최종 업데이트
            - `python -m build` 재실행
            - 새 `venv`에서 wheel 설치 테스트 최종 통과

         2. 업로드/배포
            - 사내 레지스트리 또는 PyPI에 업로드
            - 배포 후 완전 신규 `venv`에서 `pip install <배포이름>=0.1.0` + quickstart 실행(최종 확인)

         3. 태그/릴리스 노트
            - Git 태그 `v0.1.0`로 소스 스냅샷 고정
            - 릴리스 노트에 추가/변경/알려진 이슈 명시

   7. `v0.1.0` 이후는 버전 A와 동일한 반복 루프

      버전 B는 첫 공식 배포 전 전략이고 `v0.1.0`이 나가면 이후는 표준 릴리스 반복이다.

      > 기능 추가 -> 테스트 -> 빌드/설치 검증 -> `v0.2.0` 배포 -> 반복
...
프로젝트 저장소를 외부에 공유하려면 04, 05에서 작업한 콘텐츠를 저장소의 루트 디렉토리로 이동하고, 이전 디렉토리들은 모두 삭제해야 한다. 

이러한 변경을 적용하기에 앞서, 현재까지의 작업 구조를 보존하기 위해 별도의 GitHub 브랜치를 생성해두는 것이 좋다.

이 새로운 브랜치는 기존 main 브랜치를 기반으로 하며, 학습용으로 사용한 디렉토리 구조를 그대로 유지할 수 있다.

```
git checkout -b learning-branch
git push -u origin learning-branch
```